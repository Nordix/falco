...
void options::define(cxxopts::Options& opts)
{
	opts.add_options()
		...
		("e,disable-entry-events",   "Disable entry events for all syscalls except mandatory syscalls such as TOCTTOU mitigation.", cxxopts::value(disable_entry_events)->default_value("false"))
#ifdef HAS_GVISOR
		...
		("t,disable-tocttou",        "Disable TOCTTOU support by omitting TOCTTOU entry events. This option has no effect if entry events are not also disabled.", cxxopts::value(disable_tocttou)->default_value("false"))
